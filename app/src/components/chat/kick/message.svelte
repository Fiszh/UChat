<script lang="ts">
    ...
    const username = (message.user.toLowerCase().trim() ||
        "") as Lowercase<string>;
    const nameColor = message.identity.color
        ? fixNameColor(message.identity.color)
    ...
    $: if (chatSettings?.fadeOut && window.location.search) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                messages.update((e) =>
                    e.filter((msg) => msg.identity.id != message.id),
                );
            }, 2600);
        }, delay);
    }
    ...
    let parsedBadges = parseBadges(identity);
    ...
    if (!window.location.search) {
        ...
        badges.subscribe(async () => (parsedBadges = parseBadges(identity)));
    ...
    const unsubscribeCosmetics = cosmetics.subscribe(async () => {
        parsedBadges = parseBadges(identity);
        userPaint = getPaint(username);
    });
