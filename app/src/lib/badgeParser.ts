...
export function parseBadges(
  ...
  if (userstate["badges-raw"] && Object.keys(userstate["badges-raw"]).length) {
    ...
    for (const Badge of badgesSplit) {
      ...
      if (badgeSplit[0] === "subscriber" && badges_data["TTV"].sub) {
        ...
        if (userstate?.badges?.["subscriber"]) {
          const badge = badges_data["TTV"].sub.find(
            (badge: Badge) => badge.id === userstate.badges["subscriber"],
          ) as Badge | undefined;
      ...
      const badge = badges_data["TTV"].global.find(
        (badge: Badge) => badge.id === `${badgeSplit[0]}_${badgeSplit[1]}`,
      ) as Badge | undefined;
  ...
  const foundFFZBadge = badges_data["FFZ"]["global"].find(
    (badge: Record<string, any>) =>
      badge.id == badges_data["FFZ"]["user"]["user"][userstate["user-id"]],
  );
  ...
  const foundBTTVBadge = badges_data["BTTV"]["global"].find(
    (badge: Record<string, any>) => badge.providerId == userstate?.["user-id"],
  );
